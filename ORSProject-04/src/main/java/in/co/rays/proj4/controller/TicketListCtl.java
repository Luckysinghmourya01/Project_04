...
public class TicketListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<TicketBean> list = model.search(bean, pageNo, pageSize);

			List<TicketBean> next = model.search(bean, pageNo + 1, pageSize);
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					for (String id : ids) {
						...
						ServletUtility.setSuccessMessage("Ticket deleted sucessfully", request);
					}
				} else {
	...
	protected String getView() {
		return ORSView.TICKET_LIST_VIEW;
	}
