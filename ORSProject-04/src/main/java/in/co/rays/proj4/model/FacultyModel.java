...
public class FacultyModel {
	...
	public Integer nextPk() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstm = conn.prepareStatement("select max(id) from st_faculty");
		ResultSet rs = 	pstm.executeQuery();
		while(rs.next()) {
		pk = 	rs.getInt(1);
		}
		}catch(Exception e) {
			throw new DatabaseException("exception is getting pk faculty");
		}
		finally {
	...
	public long add(FacultyBean bean) throws ApplicationException, DublicateRecordException {
		...
		CollageBean collegeBean = collegeModel.findByPk(bean.getCollegeId());
		...
		if (existbean != null) {
			throw new DublicateRecordException("Email Id already exists");
		}
	...
	public void update(FacultyBean bean) throws ApplicationException, DublicateRecordException {
		...
		if (beanExist != null && !(beanExist.getId() == bean.getId())) {
			throw new DublicateRecordException("EmailId is already exist");
		}
		try {
			conn = JDBCDataSource.getConnection();
